...
CREATE OR REPLACE FUNCTION public.get_public_credential(_share_token text)
 RETURNS TABLE(id uuid, title text, earner_name text, issuer_name text, issued_at timestamp with time zone, expires_at timestamp with time zone, status credential_status, credential_lifecycle text, source learning_source, level cred_level, ects numeric, skills text[], grade text, ebsi_status text, qa_type text, qa_document_path text, qa_document_paths text[], prerequisites text, prerequisites_none boolean, supervision_type text, stackability_type text, vc_id text, template_version text, credential_hash text, learner_commitment text, template_ref text, chain_status text, chain_tx_hash text, chain_block_number bigint, chain_issuer_address text, chain_contract_address text)
 LANGUAGE sql
