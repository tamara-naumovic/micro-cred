...
/**
 * Issue one credential from an approved application. Wraps issueCredentialsBatch
 * to guarantee chain fields (vc_id / credential_hash / learner_commitment /
 * learner_secret / template_ref) are populated, then updates application status
 * and writes a timeline entry. Replaces the legacy client-side path in store.tsx.
 */
export const issueCredentialFromApplication = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator(
    (d: {
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: app, error: appErr } = await supabase
      .from("applications")
      .select("id, template_id, earner_id, issuer_id, status")
      .eq("id", data.applicationId)
      .maybeSingle();
    if (appErr) throw new Error(appErr.message);
    if (!app) throw new Error("Application not found");

    // Permission: same as issuance — issuer_admin in org, platform_admin, or template assignee
    const { data: isAdmin } = await supabase.rpc("is_platform_admin", { _user_id: userId });
    const { data: isOrgAdmin } = await supabase.rpc("has_role_in_org", {
      _user_id: userId,
      _role: "issuer_admin",
      _org_id: (app as any).issuer_id,
    });
    const { data: isAssignee } = await supabase.rpc("is_template_assignee", {
      _user_id: userId,
      _template_id: (app as any).template_id,
    });
    if (!isAdmin && !isOrgAdmin && !isAssignee) throw new Error("Forbidden");
    ...
    const { data: earnerProfile } = await supabase
      ...
      .eq("id", (app as any).earner_id)
      .maybeSingle();
    const earnerName =
      (earnerProfile as any)?.display_name ||
      ((earnerProfile as any)?.email as string | undefined)?.split("@")[0] ||
      "Earner";

    // Delegate to the batch issuance path (which always populates chain fields).
    const batchResult = await (issueCredentialsBatch as unknown as (args: {
      data: {
        templateId: string;
        issuedAt: string;
        recipients: IssueRecipientInput[];
    ...
    }) => Promise<{ ok: boolean; results: { credentialId?: string; error?: string }[] }>)({
      data: {
        templateId: (app as any).template_id,
        issuedAt: data.issuedAt ?? new Date().toISOString(),
        recipients: [
          {
            earnerId: (app as any).earner_id,
            earnerName,
            grade: data.grade ?? null,
            expiresAt: data.expiryDate ?? null,
          },
        ],
        anchorMode: data.anchorMode ?? "later",
      },
    });

    const first = batchResult.results[0];
    if (!first?.credentialId) {
      throw new Error(first?.error || "Issuance failed");
    }

    await supabase
      .from("applications")
      .update({ status: "issued", resulting_credential_id: first.credentialId } as never)
      .eq("id", data.applicationId);
    ...
    return { ok: true, credentialId: first.credentialId };
  });
