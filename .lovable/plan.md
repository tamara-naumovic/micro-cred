...
- Upozorenje 2 minuta pre isteka: nenametljivo obaveštenje („Bićete odjavljeni za 2 minuta zbog neaktivnosti“) sa dugmetom „Ostani prijavljen“.
- Po isteku: odjava, čišćenje keša podataka i preusmerenje na `/login` sa porukom da je sesija istekla zbog neaktivnosti.
...
1. Novi hook `src/lib/use-idle-logout.ts`
   - konstante: `IDLE_MS = 45 * 60 * 1000`, `WARN_MS = 2 * 60 * 1000`
   ...
   - jedan interval (svakih ~15s) upoređuje trenutno vreme sa poslednjom aktivnošću i pokreće upozorenje odnosno odjavu
...
4. Upozorenje i poruka
   - `sonner` toast za upozorenje sa akcijom „Ostani prijavljen“
   - na `/login` prikaz poruke kada je `reason=idle`

5. Lokalizacija (EN/SR)
   - novi ključevi u `common.json`: `idle.warningTitle`, `idle.warningBody`, `idle.staySignedIn`, `idle.expired`
